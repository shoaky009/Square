...
public sealed class MemoryNavigationHistory : INavigationHistory
{
    ...
    public MemoryNavigationHistory(string initialLocation = "/") => _entries = [Normalize(initialLocation)];
    ...
    private static string Normalize(string value) =>
        string.IsNullOrWhiteSpace(value) ? "/" : value.Trim().StartsWith('/') ? value.Trim() : "/" + value.Trim();
}
