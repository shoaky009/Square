...
/// <summary>
/// 多行代码编辑控件：PieceTable 模型、视口绘制、Monarch 高亮、VS Code 风格语言配置。
/// </summary>
public sealed class CodePad : UIElement, ITextEditor
{
    ...
    private readonly CodePadTextModel _model = new();
    private readonly FoldingEngine _folding = new();
    private readonly CodePadViewLayout _viewLayout = new();
    private readonly Dictionary<string, CodePadLineDecoration> _decorations = new(StringComparer.Ordinal);
    private readonly Dictionary<int, List<CodePadLineDecoration>> _decorationsByLine = [];
    /// <summary>附加光标（不含主光标 _caretIndex/_selectionAnchor）。</summary>
    private readonly List<CodePadCursor> _extraCursors = [];
    ...
    public ICodePadTextModel Model => _model;
    ...
    public event EventHandler<CodePadGutterClickEventArgs>? GutterClick;

    /// <summary>选区或光标位置变化（含折叠后仍保持的文档选区）。</summary>
    public event EventHandler? SelectionChanged;

    /// <summary>设置或替换行装饰（同 Id 覆盖）。</summary>
    public void SetDecoration(CodePadLineDecoration decoration)
    {
    ...
    public void SetDecorations(IEnumerable<CodePadLineDecoration> decorations)
    {
    ...
    public IReadOnlyList<CodePadLineDecoration> GetDecorations()
        => _decorations.Values.OrderBy(d => d.Line).ThenBy(d => d.Id, StringComparer.Ordinal).ToArray();

    /// <summary>读取指定行的装饰。</summary>
    public IReadOnlyList<CodePadLineDecoration> GetDecorationsAt(int line)
        => _decorationsByLine.TryGetValue(line, out var list) ? list : Array.Empty<CodePadLineDecoration>();
    ...
    public IReadOnlyList<CodePadCursor> Cursors
    {
        get
        {
            var list = new List<CodePadCursor>(1 + _extraCursors.Count)
            {
    ...
    public bool AddCursor(int offset)
    {
        ...
        _extraCursors.Add(CodePadCursor.Collapsed(offset));
        DeduplicateCursors();
        InvalidatePaint();
        NotifySelectionChanged();
        return true;
    ...
    public void SetCursors(IEnumerable<CodePadCursor> cursors)
    {
    ...
    {
        EnsureFolds();
        var theme = CodePadThemeRegistry.Get(ThemeId);
        ...
        context.PushClip(new Rect(
            ...
            Geometry.Y + 1,
            Math.Max(0, Geometry.Width - leftGutter - rightGutter - 2),
        ...
        for (var rowIndex = firstRow; rowIndex <= lastRow; rowIndex++)
        {
            ...
            {
                context.FillRect(
                    new Rect(Geometry.X + 1 + leftGutter, y, Math.Max(0, Geometry.Width - 2 - leftGutter - rightGutter), lineHeight),
                    new SolidColorBrush(theme.EditorCurrentLineBackground));
            }

            if (row.IsFirstOfDocumentLine)
                PaintDecorationLineBackground(context, line, Geometry.X + 1 + leftGutter, y, Math.Max(0, Geometry.Width - 2 - leftGutter - rightGutter), lineHeight);
            ...
            if (ShowFolding && row.IsFirstOfDocumentLine && _folding.IsCollapsed(line))
            {
                var content = _model.GetLineContent(line);
                var endX = contentLeft + CodePadMetrics.XAtColumn(content, font, TabSize, content.Length) + 6;
                var ellipsis = new TextLayout("⋯", font);
                context.DrawText(ellipsis, new Point(endX, y), new SolidColorBrush(theme.EditorLineNumberForeground));
            }
        }
        ...
        if (addCursor && !extendSelection)
        {
            ...
            else
            {
                ...
                if (SelectionLength > 0)
                    _selectionAnchor = _caretIndex;
                _extraCursors.Add(CodePadCursor.Collapsed(index));
                DeduplicateCursors();
            }
            _dragging = false;
            ResetCaretBlink(fullRepaint: true);
    ...
    private void ApplyRestoredCarets(int[] carets, int fallback)
    {
        ...
        for (var i = 0; i < sorted.Length - 1; i++)
            _extraCursors.Add(CodePadCursor.Collapsed(sorted[i]));
    }
    ...
    private void MoveVertical(int direction, bool extend)
    {
        ...
        _preferredX ??= CodePadMetrics.XAtColumn(rowText, font, TabSize, localCol);
        ...
        var targetLocal = CodePadMetrics.ColumnAtX(targetSeg, font, TabSize, _preferredX.Value);
        ...
        float y)
    {
        ...
        if (spans.Count == 0)
        {
            ...
            DrawSegment(context, font, CodePadMetrics.ExpandTabs(slice, TabSize), x, y, theme.EditorForeground);
            return;
        }
        ...
        foreach (var span in spans)
        {
            ...
            var expanded = CodePadMetrics.ExpandTabs(segment, TabSize);
            var color = theme.ResolveTokenColor(span.Type);
            var prefixInRow = content[row.Start..from];
            var sx = x + CodePadMetrics.XAtColumn(prefixInRow, font, TabSize, prefixInRow.Length);
        ...
        float lineHeight)
    {
        ...
        // 折叠头：若选区覆盖折叠块（含隐藏行），高亮延伸到 ⋯，表示整块被选中
        if (!ShowFolding || !row.IsFirstOfDocumentLine || !_folding.IsCollapsed(row.DocumentLine))
            return;
        ...
        var content = _model.GetLineContent(row.DocumentLine);
        var lineEndX = contentLeft + CodePadMetrics.XAtColumn(content, font, TabSize, content.Length);
        var ellipsisWidth = Math.Max(12f, MeasureTextWidth("⋯", font) + 10f);
        context.FillRect(
        ...
        bool expandNewline)
    {
        ...
        var rowText = content.Length == 0 ? "" : content[row.Start..Math.Min(row.End, content.Length)];
        var x0 = contentLeft + CodePadMetrics.XAtColumn(rowText, font, TabSize, startCol);
        var x1 = contentLeft + CodePadMetrics.XAtColumn(rowText, font, TabSize, endCol);
    ...
    private int HitTestOffset(Point point)
    {
        ...
        var content = _model.GetLineContent(row.DocumentLine);
        var rowText = content.Length == 0 ? "" : content[row.Start..Math.Min(row.End, content.Length)];
        var localCol = CodePadMetrics.ColumnAtX(rowText, font, TabSize, point.X - contentLeft);
    ...
    private bool TryHandleGutterClick(Point point, bool extendSelection = false)
    {
        ...
        CodePadGutterLane lane;
        if (glyphGutter > 0 && localX < glyphGutter)
            lane = CodePadGutterLane.Glyph;
        else if (lineGutter > 0 && localX < glyphGutter + lineGutter)
            lane = CodePadGutterLane.LineNumbers;
        else
            lane = CodePadGutterLane.Folding;

        if (lane == CodePadGutterLane.Folding && ShowFolding && row.IsFirstOfDocumentLine && _folding.CanFoldAt(line))
        {
            // Shift+点击折叠槽：选中折叠内容（不切换折叠）
            if (extendSelection)
            {
                SelectCollapsedFoldAt(line);
                GutterClick?.Invoke(this, new CodePadGutterClickEventArgs(line, point, lane));
                return true;
            }
            ToggleFoldAt(line);
        }
        ...
        float lineHeight)
    {
        ...
        Color? barColor = null;
        CodePadLineDecoration? glyphDecoration = null;
        foreach (var decoration in list)
        {
    ...
    {
        EnsureFolds();
        var contentWidth = Math.Max(1, Geometry.Width - padding * 2 - gutter - GetRightGutterWidth());
        EnsureViewLayout(font, contentWidth);
        var (line, col) = _model.GetPositionAt(_caretIndex);
        ...
        var x = Geometry.X + padding + gutter - (WordWrap ? 0f : _scrollX)
                + CodePadMetrics.XAtColumn(rowText, font, TabSize, localCol);
    ...
    private void EnsureCaretVisible()
    {
        ...
        if (!WordWrap)
        {
            ...
            var localCol = Math.Clamp(col - row.Start, 0, Math.Max(0, row.End - row.Start));
            var rowText = content.Length == 0 ? "" : content[row.Start..Math.Min(row.End, content.Length)];
            var caretX = CodePadMetrics.XAtColumn(rowText, font, TabSize, localCol);
            if (caretX < _scrollX)
                _scrollX = caretX;
    ...
    private void EnsureScroll(float lineHeight, float contentHeight, Font? font = null, float? contentWidth = null)
    {
        ...
        // Prefer caller viewport if provided (keeps paint path stable), else use metrics.
        var viewportHeight = contentHeight > 0 ? contentHeight : metrics.ViewportHeight;
        var viewportWidth = contentWidth ?? metrics.ViewportWidth;
        EnsureViewLayout(font, WordWrap ? Math.Max(1, viewportWidth) : Math.Max(1, metrics.ViewportWidth));
    ...
    private float MeasureMaxContentWidth(Font font)
    {
        ...
        for (var line = 0; line < _model.LineCount; line++)
        {
            ...
            var content = _model.GetLineContent(line);
            max = Math.Max(max, CodePadMetrics.MeasureLineWidth(content, font, TabSize));
        }
        return max + 24f;
    }
    ...
    private void OnWheel(Event e)
    {
        ...
        EnsureScroll(lineHeight, metrics.ViewportHeight, font, metrics.ViewportWidth);
        InvalidatePaint();
        e.StopPropagation();
    }

    private enum ScrollBarHit { None, VerticalTrack, VerticalThumb, HorizontalTrack, HorizontalThumb }

    private readonly record struct ScrollMetrics(
        float ViewportWidth,
    ...
    private ScrollMetrics GetScrollMetrics(Font font, float lineHeight)
    {
        var padding = DefaultPadding;
        var leftGutter = GetLeftGutterWidth();
        var rightGutter = GetRightGutterWidth();
        ...
        EnsureViewLayout(font, WordWrap ? baseWidth : baseWidth); // width may shrink if V bar shown

        var contentHeight = _viewLayout.RowCount * lineHeight;
        var contentWidth = WordWrap ? baseWidth : MeasureMaxContentWidth(font);

        var needV = contentHeight > baseHeight + 0.5f;
        var needH = !WordWrap && contentWidth > baseWidth + 0.5f;
        // corner: both bars reduce viewport
        var viewportWidth = baseWidth - (ShowScrollBars && needV ? ScrollBarThickness : 0);
        var viewportHeight = baseHeight - (ShowScrollBars && needH ? ScrollBarThickness : 0);
        if (WordWrap && ShowScrollBars && needV)
        {
            // re-wrap with reduced width
            EnsureViewLayout(font, Math.Max(1, viewportWidth));
            contentHeight = _viewLayout.RowCount * lineHeight;
            needV = contentHeight > viewportHeight + 0.5f;
            viewportWidth = baseWidth - (ShowScrollBars && needV ? ScrollBarThickness : 0);
            EnsureViewLayout(font, Math.Max(1, viewportWidth));
            contentHeight = _viewLayout.RowCount * lineHeight;
        }

        var maxScrollY = Math.Max(0, contentHeight - viewportHeight);
        var maxScrollX = WordWrap ? 0f : Math.Max(0, contentWidth - viewportWidth);

        var vTrack = Rect.Empty;
        var hTrack = Rect.Empty;
        var vThumb = Rect.Empty;
        var hThumb = Rect.Empty;
        if (ShowScrollBars && needV)
        {
            var x = Geometry.Right - rightGutter - ScrollBarThickness - 1;
            var y = Geometry.Y + 1;
            var h = Math.Max(0, Geometry.Height - 2 - (needH ? ScrollBarThickness : 0));
            vTrack = new Rect(x, y, ScrollBarThickness, h);
            var thumbH = Math.Max(ScrollBarMinThumb, h * (viewportHeight / Math.Max(viewportHeight, contentHeight)));
            var travel = Math.Max(0, h - thumbH);
            var thumbY = y + (maxScrollY <= 0 ? 0 : travel * (_scrollY / maxScrollY));
            vThumb = new Rect(x + 1, thumbY, ScrollBarThickness - 2, thumbH);
        }
        if (ShowScrollBars && needH)
        {
            var x = Geometry.X + leftGutter + 1;
            var y = Geometry.Bottom - ScrollBarThickness - 1;
            var w = Math.Max(0, Geometry.Width - leftGutter - rightGutter - 2 - (needV ? ScrollBarThickness : 0));
            hTrack = new Rect(x, y, w, ScrollBarThickness);
            var thumbW = Math.Max(ScrollBarMinThumb, w * (viewportWidth / Math.Max(viewportWidth, contentWidth)));
            var travel = Math.Max(0, w - thumbW);
            var thumbX = x + (maxScrollX <= 0 ? 0 : travel * (_scrollX / maxScrollX));
            hThumb = new Rect(thumbX, y + 1, thumbW, ScrollBarThickness - 2);
        }

        return new ScrollMetrics(
            viewportWidth, viewportHeight, contentWidth, contentHeight,
            maxScrollX, maxScrollY, needV, needH, vTrack, hTrack, vThumb, hThumb);
    }
    ...
    {
        var metrics = GetScrollMetrics(font, lineHeight);
        if (!metrics.NeedV && !metrics.NeedH) return;
        ...
        if (metrics.NeedV && !metrics.VTrack.IsEmpty)
        {
            context.FillRect(metrics.VTrack, new SolidColorBrush(track));
            context.FillRect(metrics.VThumb, new SolidColorBrush(_draggingVScroll ? thumbActive : thumb));
        }
        if (metrics.NeedH && !metrics.HTrack.IsEmpty)
        {
            context.FillRect(metrics.HTrack, new SolidColorBrush(track));
            context.FillRect(metrics.HThumb, new SolidColorBrush(_draggingHScroll ? thumbActive : thumb));
        }
        if (metrics.NeedV && metrics.NeedH)
        {
            var rightGutter = GetRightGutterWidth();
            context.FillRect(
                new Rect(Geometry.Right - rightGutter - ScrollBarThickness - 1, Geometry.Bottom - ScrollBarThickness - 1, ScrollBarThickness, ScrollBarThickness),
                new SolidColorBrush(track));
        }
    }
    ...
    private ScrollBarHit HitTestScrollBar(Point point)
    {
        ...
        if (metrics.NeedV)
        {
            if (metrics.VThumb.Contains(point)) return ScrollBarHit.VerticalThumb;
            if (metrics.VTrack.Contains(point)) return ScrollBarHit.VerticalTrack;
        }
        if (metrics.NeedH)
        {
            if (metrics.HThumb.Contains(point)) return ScrollBarHit.HorizontalThumb;
            if (metrics.HTrack.Contains(point)) return ScrollBarHit.HorizontalTrack;
        }
    ...
    private bool TryHandleScrollBarPointerDown(Point point)
    {
        ...
        switch (hit)
        {
            ...
            {
                var page = metrics.ViewportHeight * 0.9f;
                if (point.Y < metrics.VThumb.Y)
                    _scrollY = Math.Max(0, _scrollY - page);
                else
                    _scrollY = Math.Min(metrics.MaxScrollY, _scrollY + page);
                EnsureScroll(lineHeight, metrics.ViewportHeight, font, metrics.ViewportWidth);
                break;
            }
            case ScrollBarHit.HorizontalTrack:
            {
                var page = metrics.ViewportWidth * 0.9f;
                if (point.X < metrics.HThumb.X)
                    _scrollX = Math.Max(0, _scrollX - page);
                else
                    _scrollX = Math.Min(metrics.MaxScrollX, _scrollX + page);
                EnsureScroll(lineHeight, metrics.ViewportHeight, font, metrics.ViewportWidth);
                break;
            }
        }
    ...
    private void HandleScrollBarDrag(Point point)
    {
        ...
        if (_draggingVScroll && metrics.NeedV && metrics.MaxScrollY > 0)
        {
            var travel = Math.Max(1, metrics.VTrack.Height - metrics.VThumb.Height);
        ...
        else if (_draggingHScroll && metrics.NeedH && metrics.MaxScrollX > 0)
        {
            var travel = Math.Max(1, metrics.HTrack.Width - metrics.HThumb.Width);
    ...
    private void ApplyEditToAllCursors(string insert, bool isBackspace, bool isDelete)
    {
        ...
        var cursors = new List<CodePadCursor>(1 + _extraCursors.Count) { new(_caretIndex, _selectionAnchor) };
        ...
        for (var i = 0; i < finalCarets.Count - 1; i++)
            _extraCursors.Add(CodePadCursor.Collapsed(finalCarets[i]));
        AfterEdit();
        NotifySelectionChanged();
    }

    private static void DeduplicateCursorList(List<CodePadCursor> list)
    {
    ...
    {
        var all = new List<CodePadCursor>(1 + _extraCursors.Count) { new(_caretIndex, _selectionAnchor) };
    ...
    private CodePadCursor MoveCursorHorizontal(CodePadCursor c, int direction, bool byWord, bool extend)
    {
        ...
        if (!extend && !c.IsCollapsed)
        {
            ...
            return CodePadCursor.Collapsed(edge);
        }
        ...
        return new CodePadCursor(next, anchor).Clamp(_model.Length);
    }
    ...
    private CodePadCursor MoveCursorVertical(CodePadCursor c, int direction, bool extend, Font font)
    {
        if (!extend && !c.IsCollapsed)
        {
            var edge = direction < 0 ? c.SelectionStart : c.SelectionStart + c.SelectionLength;
        ...
        var localCol = Math.Clamp(col - row.Start, 0, Math.Max(0, row.End - row.Start));
        var rowText = content.Length == 0 ? "" : content[row.Start..Math.Min(row.End, content.Length)];
        var preferredX = CodePadMetrics.XAtColumn(rowText, font, TabSize, localCol);
        var targetRowIndex = Math.Clamp(rowIndex + direction, 0, _viewLayout.RowCount - 1);
        int next;
        if (targetRowIndex == rowIndex)
        {
        ...
        else
        {
            ...
            var targetSeg = targetContent.Length == 0
                ? ""
                : targetContent[targetRow.Start..Math.Min(targetRow.End, targetContent.Length)];
            var targetLocal = CodePadMetrics.ColumnAtX(targetSeg, font, TabSize, preferredX);
    ...
    private void MoveAllCursorsToLineBoundary(bool toStart, bool control, bool extend)
    {
        EnsureFolds();
        var font = ResolveFont();
        EnsureViewLayout(font, GetContentWidth());
        var all = new List<CodePadCursor>(1 + _extraCursors.Count) { new(_caretIndex, _selectionAnchor) };
        all.AddRange(_extraCursors);
        for (var i = 0; i < all.Count; i++)
        {
            ...
            if (!extend && !c.IsCollapsed)
            {
                ...
                all[i] = CodePadCursor.Collapsed(edge);
                continue;
            }
            ...
            var anchor = extend ? c.Anchor : next;
            all[i] = new CodePadCursor(next, anchor).Clamp(_model.Length);
        }
        ApplyCursorList(all);
    }

    private void ApplyCursorList(List<CodePadCursor> all)
    {
