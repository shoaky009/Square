...
public readonly record struct CodeEditorCursor(int Caret, int Anchor)
{
    /// <summary>折叠为单点光标。</summary>
    public static CodeEditorCursor Collapsed(int offset) => new(offset, offset);
    ...
    public CodeEditorCursor Clamp(int documentLength)
    {
        var len = Math.Max(0, documentLength);
        return new CodeEditorCursor(Math.Clamp(Caret, 0, len), Math.Clamp(Anchor, 0, len));
    }
}
