...
internal readonly record struct CodeEditorViewRow(
    int DocumentLine,
    int Start,
    int End,
...
internal sealed class CodeEditorViewLayout
{
    private readonly List<CodeEditorViewRow> _rows = [];
    ...
    public CodeEditorViewRow this[int index] => _rows[Math.Clamp(index, 0, Math.Max(0, _rows.Count - 1))];
    ...
    public int OffsetToRow(ICodeEditorTextModel model, int offset)
    {
        ...
        bool wordWrap)
    {
        ...
        for (var v = 0; v < visible; v++)
        {
            ...
            if (folding.IsCollapsed(line) && folding.GetRegionStartingAt(line) is { StartColumn: >= 0 } fold)
                content = content[..Math.Min(fold.StartColumn, content.Length)];
            if (_docLineToFirstRow[line] < 0)
                _docLineToFirstRow[line] = _rows.Count;

            if (!wordWrap || content.Length == 0)
            {
                _rows.Add(new CodeEditorViewRow(line, 0, content.Length, true));
                continue;
            }

            var segments = WrapLine(content, font, tabSize, maxWidth);
            for (var s = 0; s < segments.Count; s++)
                _rows.Add(new CodeEditorViewRow(line, segments[s].Start, segments[s].End, s == 0));
        }

        if (_rows.Count == 0)
            _rows.Add(new CodeEditorViewRow(0, 0, 0, true));
    }
